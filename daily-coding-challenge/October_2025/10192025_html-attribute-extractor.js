/*
(October 19, 2025) HTML Attribute Extractor
...
Test:
extractAttributes('<span class="red"></span>') => ["class, red"]
extractAttributes('<button id="submit" class="btn btn-primary">Submit</button>') => ["id, submit", "class, btn btn-primary"]
*/

function extractAttributes(element) {
  const match = element.match(/([\w-]+)="([^"]*)"/g);
  if (match) return match.map(data => data.replace(/"/g, '').split('=').join(', '));
  else return [];
}
